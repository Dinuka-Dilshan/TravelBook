...
const SignUp = () => {
  ...
  const { field, inputHandler, resetHandler, state, touchHandler } = useForm({
    ...
    fields: {
      ...
      phoneNumber: {
        ...
        validators: [
          {
            validator: (val) => isMobilePhone(val,'si-LK'),
            errorMessage: "Invalid Phone Number",
          },
        ],
        isTouched: false,
      ...
      nationalID: {
        ...
        validators: [
          {
            // @ts-ignore
            validator: (val) => isIdentityCard(val,'LK'),
            errorMessage: "Invalid national ID",
          },
        ],
        isTouched: false,
  ...
  const handleJoin = () => {
    if (state.isValid) {
      ...
      formData.append("phoneNumber", field("phoneNumber").value);
      formData.append("nationalID", field("nationalID").value);
      formData.append("profilePicture", image as Blob);
      fetchData("user/business/signup", {
        method: "POST",
        body: formData,
        type: "file",
    ...
    <Container>
      ...
      >
        ...
        >
          ...
          />
          {!field("phoneNumber").isValid && field("phoneNumber").isTouched && (
            <Typography pt="0.5rem" color={"custom.red"}>
              {field("phoneNumber").errorMessage}
            </Typography>
          )}
          <TextField
            size="small"
            fullWidth
            onBlur={() => {
              touchHandler("nationalID");
            }}
            label="National ID"
            value={field("nationalID").value}
            onChange={(e) => {
              inputHandler("nationalID", e.target.value);
            }}
          />
          {!field("nationalID").isValid && field("nationalID").isTouched && (
            <Typography pt="0.5rem" color={"custom.red"}>
              {field("nationalID").errorMessage}
            </Typography>
          )}
