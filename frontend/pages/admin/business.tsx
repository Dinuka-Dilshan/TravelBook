...
import {
  BusinessPlacesResponse,
  BusinessPlacewithBookings,
} from "@/models/Admin";
...
const columns: GridColDef<BusinessPlacewithBookings>[] = [
  {
    field: "picture",
    headerName: "",
    width: 40,
  ...
  {
    field: "income",
    headerName: "Income (LKR)",
    width: 120,
    valueGetter: (params) =>
      params.row["bookings"].reduce(
...
const BusinessPlaces: React.FC = () => {
  const {
    data: places,
    error,
    fetchData,
    isLoading,
  } = useFetch<BusinessPlacesResponse>();
  const [pageSize, setPageSize] = useState<number>(10);

  useEffect(() => {
    fetchData("admin/businessPlaces", { method: "GET", type: "authenticated" });
  }, []);
